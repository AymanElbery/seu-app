...
@Component({
  selector: 'app-academic-diplome-status',
  templateUrl: './academic-diplome-status.component.html',
  styleUrls: ['./academic-diplome-status.component.scss']
})
export class AcademicDiplomeStatusComponent implements OnInit, OnDestroy {
  ...
  constructor(private transalte: TranslateService, private academicStatusService: AcademicDiplomeStatusService) {
  ...
  getReqs() {
    ...
    this.academicStatusService.getStaudentStatus().then((res) => {
      ...
      if (res['status']) {
        ...
        this.selectedSems = this.studentTerms.length ? this.studentTerms[0].TERM_CODE : '';
